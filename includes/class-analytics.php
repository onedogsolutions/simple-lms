...
class Analytics
{
    ...
    public static function run_daily_rollup()
    {
        ...
        $enroll_rows = $wpdb->get_results($wpdb->prepare(
        ...
        $site_totals = array('enrollments' => 0, 'completions' => 0, 'certificates' => 0, 'active_students' => 0);
    ...
    private static function period_totals($from, $to)
    {
        ...
        $certificates = (int) $wpdb->get_var($wpdb->prepare(
    ...
    private static function time_series($from, $to)
    {
        ...
        $rollup = $wpdb->get_results($wpdb->prepare(
        ...
        $enroll_live = $wpdb->get_results($wpdb->prepare(
        ...
        $complete_live = $wpdb->get_results($wpdb->prepare(
    ...
    private static function csv_at_risk($days)
    {
        ...
        return array(
            ...
            'header'   => array(
                'Student', 'Email', 'Course', 'Enrolled', 'Last Activity',
                'Days Inactive', 'Access Expires', 'Days Until Expiry', 'Reasons',
            ),
            'rows'     => $rows,
        );
    }
