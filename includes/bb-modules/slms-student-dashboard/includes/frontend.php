	...
	<ul class="slms-dash-tabs">
		<li class="<?php echo ('profile' === $current_tab) ? 'active' : ''; ?>">
			<a href="<?php echo esc_url(add_query_arg('dash_tab', 'profile', $url)); ?>">
				...
				<div class="slms-dash-label-group">
					<strong>
						<?php echo esc_html($tab_label_profile); ?>
					</strong>
					<span>
						<?php _e('Update your account information.', 'simple-lms-bridge'); ?>
					</span>
				</div>
			</a>
		</li>
		<li class="<?php echo ('history' === $current_tab) ? 'active' : ''; ?>">
			<a href="<?php echo esc_url(add_query_arg('dash_tab', 'history', $url)); ?>">
				...
				<div class="slms-dash-label-group">
					<strong>
						<?php echo esc_html($tab_label_history); ?>
					</strong>
					<span>
						<?php _e('View your course purchases.', 'simple-lms-bridge'); ?>
					</span>
				</div>
			</a>
		</li>
		<li class="<?php echo ('certificates' === $current_tab) ? 'active' : ''; ?>">
			<a href="<?php echo esc_url(add_query_arg('dash_tab', 'certificates', $url)); ?>">
				...
				<div class="slms-dash-label-group">
					<strong>
						<?php echo esc_html($tab_label_certs); ?>
					</strong>
					<span>
						<?php _e('Download your certificates.', 'simple-lms-bridge'); ?>
					</span>
				</div>
			</a>
		</li>
...
?>
		...
		<?php
elseif ('certificates' === $current_tab): ?>
		...
		if (!is_wp_error($entries) && !empty($entries)): ?>
			<table class="slms-dash-table">
				<thead>
					<tr>
						<th>
							<?php _e('Name', 'simple-lms-bridge'); ?>
						</th>
						<th>
							<?php _e('Course', 'simple-lms-bridge'); ?>
						</th>
						<th>
							<?php _e('Completion Date', 'simple-lms-bridge'); ?>
						</th>
						<th>
							<?php _e('Certificate PDF', 'simple-lms-bridge'); ?>
						</th>
					</tr>
				</thead>
				<tbody>
					...
					<tr>
						<td>
							<?php echo esc_html(rgar($entry, $settings->cert_field_name)); ?>
						</td>
						<td>
							<?php echo esc_html(rgar($entry, $settings->cert_field_course)); ?>
						</td>
						<td>
							<?php echo esc_html(date_i18n(get_option('date_format'), strtotime(rgar($entry, $settings->cert_field_date)))); ?>
						</td>
						<td>
							<?php
			...
			<p>
				<?php _e('No certificates found.', 'simple-lms-bridge'); ?>
			</p>
			<?php
		endif;
	}
