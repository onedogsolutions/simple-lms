...
## Stage 1 — Content Guarding & Student Access Model

- **`Access` authority (`includes/class-access.php`):** single source of truth
  for view decisions. `Access::can_view($user_id, $post_id)` resolves lesson →
  course (`Relationships::get_courses_for_lesson`), reads guard mode
  `_lms_guard_mode` (`public | enrolled | level`, default `enrolled`), checks
  enrollment / PMPro level, applies `_lms_access_days` expiration, and returns
  through the now-wired `simple_lms_check_access` filter (finally connecting the
  PMPro override registered in `class-pmpro.php`). Editors bypass via
  `edit_post`. `Access::denial_reason()` returns
  `not_logged_in | not_enrolled | expired`.
- **Enforcement in three layers (`includes/class-guard.php`):**
  `template_redirect` guard on course/lesson singles (logged-out → login or
  checkout per Settings; logged-in → PMPro checkout for the course's first
  mapped level, fallback levels page, with a `slms_return` param); `the_content`
  @99 excerpt + CTA fallback for archives/search/builder contexts (bypassed when
  `FLBuilderModel::is_builder_active()`); and `rest_prepare_slms_course` /
  `rest_prepare_slms_lesson` filters that strip `content.rendered` from the
  public REST API for users failing `Access::can_view()`.
- **Guard configuration:** registered `_lms_guard_mode`, `_lms_denial_behavior`
  (`redirect | message`), and `_lms_checkout_override` post meta (REST-exposed)
  in `class-cpt.php`; added an "Access Control" panel to `CourseEditor.js`.
- **Student-scoped REST namespace:** `GET/POST /me/progress` and
  `GET /me/courses`, all with `permission_callback = is_user_logged_in`. The
  user is taken from the session, so a forged `user_id` in the payload is
  ignored; enrollment (`Access`) and lesson∈course (join table) are validated
  before writing. `assets/js/frontend.js` and the `lms-complete-button` module
  now target `/me/progress` and no longer emit `data-user-id`.
- **Progress table (`wp_slms_lesson_progress`):** new queryable table created by
  schema step 2 of the Stage 0 `Upgrade` runner (`SLMS_DB_VERSION` bumped to 2).
  `includes/class-progress.php` dual-writes the table and the legacy
  `_lms_progress` meta; readers (`get_students`, `get_progress`, `lms-outline`,
  `lms-complete-button`, `Certificates::check_course_completion`) read the table
  with a per-scope meta fallback during the migration window. Idempotent backfill
  via WP-CLI (`wp slms progress-backfill`) and a Settings → Tools button. Access
  revocation (de-enroll, expiration) clears the table via `Progress::clear_course`.
- **Settings (`includes/class-settings.php`):** `slms_settings` option + REST
  routes + a React "Settings" screen (default guard mode, checkout/levels page,
  login redirect behavior, and certificate Gravity Forms field IDs that replace
  the hardcoded `6`/`18` in `class-certificates.php` and `class-rest.php`).
