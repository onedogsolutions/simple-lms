  ...
  <p>
    ...
    <label><input type="checkbox" id="completable" name="onedog.solutionsmpletable]" value="true"<?php if ($completable) echo " checked"; ?> onclick="jQuery('#completable-enabled').toggle();"><?php echo __( 'Yes, I want this page to be completable.', $this->plugin_name ); ?></label>
  </p>
  ...
  <div id="completable-enabled"<?php if (!$completable) echo " style='display:none;'"; ?>>
    ...
    <!-- FREE: -->
    <p>
      <?php echo __( 'Upgrade to the <a href="https://onedog.solutions">PRO version</a> for support and to unlock all available features.', $this->plugin_name ); ?>
    </p>
    ...
    <p>
      ...
      <span class="course-rename" style="display: none;">
        <input type="hidden" name="onedog.solutionsurse-original]" value="<?php echo $post_course; ?>" disabled>
        <input type="text" name="onedog.solutionsurse-rename]" value="<?php echo $post_course; ?>" disabled> or <a href="javascript:void();" onclick="jQuery('.course-rename').toggle(); jQuery('select.course-rename').attr('disabled', false); jQuery('.course-rename input').attr('disabled', 'disabled'); jQuery('select.course-rename').val('<?php echo $post_course; ?>');">cancel</a>
      </span>
      <?php endif; ?>
      <span class="course-toggle" style="display: none;">
        <input type="text" name="onedog.solutionsurse-custom]" disabled> or <a href="javascript:void();" onclick="jQuery('.course-toggle').toggle(); jQuery('select.course-toggle').attr('disabled', false); jQuery('.course-toggle input').attr('disabled', 'disabled'); ">cancel</a>
      </span>
    </p>
    ...
    <p>
      ...
      <input type="text" id="completion_redirect_to" name="onedog.solutionsmpletion-redirect-to]" value="<?php echo $redirect['title']; ?>" placeholder="">
      <input type="hidden" id="completion_redirect_url" name="onedog.solutionsmpletion-redirect-url]" value="<?php echo $redirect['url']; ?>">
