<div class="wrap">
  ...
  <h1>Completion:
    <?php echo (empty($button)) ? 'Default Button' : "Button '$button'"; ?>
  </h1>
  <h3>Post:
    <?php echo (is_object($post) && isset($post->post_title)) ? $post->post_title : 'Unknown Post'; ?>
  </h3>

  <div class="tablenav top">
    <div class="tablenav-pages one-page">
      <span class="displaying-num">
        <?php echo count($user_completed); ?> /
        <?php echo count($total_users); ?> completed
      </span>
    </div>
    <br class="clear">
  </div>
  ...
  <table class="wp-list-table widefat fixed striped users">
    ...
    <tbody id="the-list" data-wp-lists='list:users'>
      ...
      <tr id='user-<?php echo $user->ID; ?>'>
        <td class='name column-name' data-colname="Name"><a
            href="users.php?page=simple-lms-users&amp;user_id=<?php echo $user->ID; ?>">
