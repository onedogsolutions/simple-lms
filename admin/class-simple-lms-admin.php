...
class SimpleLMS_Admin extends SimpleLMS_Common {
  ...
  public function enqueue_scripts() {
    ...
    if ( !in_array( $pagenow, array( 'edit-tags.php' ) ) ) {
      ...
      wp_localize_script( $this->plugin_name, SIMPLE_LMS_PRODUCT_NAME, $params );
    }
  }
  ...
  public function add_options_page() {
    ...
    $this->plugin_screen_hook_suffix = add_options_page(
      ...
      __( SIMPLE_LMS_PRODUCT_NAME, $this->plugin_name ),
      'manage_options',
      $this->plugin_name,
  ...
  public function settings_role_cb() {
    ...
    $disabled = !SIMPLE_LMS_IS_ACTIVATED;
...
';
  ...
  public function add_completable_metabox() {
    ...
    add_meta_box(
      ...
      __( SIMPLE_LMS_PRODUCT_NAME, $this->plugin_name ),        // Box title
...
<?php       }
  ...
  public function show_development_mode_nag() {
    ...
    if ( !simple_lms_is_production() && // don't show in production
         !in_array( $pagenow , array('post.php', 'post_new.php') ) && // don't show on certain pages
  ...
  public function add_course_completion_page() {
    add_menu_page(
      SIMPLE_LMS_PRODUCT_NAME,
      __( SIMPLE_LMS_PRODUCT_NAME, $this->plugin_name ),
      'manage_options',
      'onedog.solutionsurses',
  ...
  public function export_course_completion_csv() {
    ...
    if ( isset( $_GET['page'] ) && in_array( $_GET['page'], array( 'onedog.solutionsurses' ) ) && isset( $_GET['export'] ) ) {
      ...
      if ( $course ) {
        header("Content-Disposition: attachment; filename=\"onedog.solutionsurse-" . $this->get_course_class(array('course' => $course)) . ".csv\";");
      } else {
        header("Content-Disposition: attachment; filename=\"onedog.solutionsurses.csv\";");
      }
  ...
  public function add_multiple_buttons_to_meta($post_id, $post_meta, $post_content = '') {
    ...
    if ( false !== strpos( $post_content, '[' ) ) {
      ...
      // We just want simple-lms buttons... can ignore anything else:
      $pattern = '\[(\[?)(complete_button|wpc_complete_button|wpc_button|simple_lms_button)(?![\w-])([^\]\/]*(?:\/(?!\])[^\]\/]*)*?)(?:(\/)\]|\](?:([^\[]*+(?:\[(?!\/\2\])[^\[]*+)*+)\[\/\2\])?)(\]?)';
      ...
      $shortcodes = array_filter($shortcodes, function($value) {
        ...
        if (strstr($value, '[simple_lms_button') !== false) return true;
      ...
      foreach ($shortcodes as $code) {
        ...
        $code = str_replace('[simple_lms_button', '[wpc_complete_button', $code);
