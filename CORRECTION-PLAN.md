...
> **Status (2026-07-18):** historical record. C1–C6 are executed and on `main`
> (guarding, progress table + backfill, analytics re-point, core/migrator
> split, PHPCS gate — see `CHANGELOG.md` v1.1.0/v1.1.1). C5's FluentPlayer
> branch is dropped for now (would need a redo against current `main`).
> **C7, the staging smoke test, remains the open gate.**
