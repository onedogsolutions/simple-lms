...
- **Stage 2 — Frontend Course Experience (Jul 2026):**
  - **Access service (`includes/class-access.php`):** New central authority for course access + state.
    - `can_view($user_id, $lesson_id, $course_id)` combines enrollment/PMPro access with drip scheduling; editors always pass (builder/preview). Filterable via `slms_can_view_lesson`.
    - Drip helpers: `get_unlock_timestamp()` / `is_dripped()` computed from `slms_user_course.enrolled_at` + lesson meta `_lms_drip_days` (falls back to legacy `_lms_enrolled_at` user meta).
    - State machine `get_course_state()` → `guest | not_enrolled | not_started | in_progress | completed`; plus `get_first_incomplete_lesson()`, `get_continue_url()`, `get_progress_stats()`, `is_course_complete()`.
    - `get_cta()` returns the state-aware CTA descriptor (label/url/classes) shared by the grid + CTA modules; `get_checkout_url()` builds the PMPro checkout URL for the course's mapped level; `format_price()` wraps `pmpro_formatPrice`.
    - `set_lesson_progress()` is the single completion code path used by both REST `/progress` and quiz auto-completion (writes `_lms_progress`, fires `Certificates::check_course_completion`).
    - `get_enrolled_courses_with_progress()` shared by `lms-my-courses` and the dashboard "My Courses" tab.
  - **New meta (`class-cpt.php`):** `_lms_drip_days` (lesson), `_lms_completion_redirect` (course), `_lms_quiz_pass_field` + `_lms_quiz_pass_min` (lesson), `_lms_video_gate_pct` (lesson). `Relationships::get_enrolled_at()` / `is_enrolled()` added.
  - **Quiz-gated completion (`includes/class-quiz.php`):** `gform_after_submission` (priority 5, before Certificates at 10) auto-completes quiz-type lessons whose `_lms_gravity_form` matches the submitted form, for every enrolled course. Optional passing-score gate via `_lms_quiz_pass_field` + `_lms_quiz_pass_min`. Routes through `Access::set_lesson_progress`. `GFAPI::add_entry` does not re-fire submission hooks, so no loop.
  - **Completion redirect:** REST `/progress` returns `redirect` (from `_lms_completion_redirect`) + `course_complete` when the course just completed — keyed off `_lms_completed_at` because certificate automation wipes `_lms_progress` on completion. Frontend JS follows the redirect.
  - **Four new self-contained BB modules** (category `SimpleLMS`, per the STATE.md architecture rule — own `css/frontend.css`, `includes/frontend.php`, `includes/frontend.css.php`, inline JS where needed):
    - `lms-course-grid` — card grid of `slms_course` (category slug filter, columns, price, enrolled badge, progress bar, state-aware CTA).
    - `lms-lesson-nav` — prev/next within `_simple_lms_order` + "Back to Course"; drip/guard-locked targets render disabled with a lock icon + unlock date.
    - `lms-my-courses` — session user's enrollments with progress bars + Continue/Start.
    - `lms-course-cta` — single state-machine button (Login/Buy · Buy · Start · Continue · View Certificate).
  - **Drip rendering:** `lms-outline` renders drip-locked lessons as non-links with lock icon + unlock date.
  - **Complete button (`lms-complete-button`):** hidden on quiz lessons (auto-complete notice); video-gated (starts `disabled` with `data-video-gate`/`data-video-id` when `_lms_video_gate_pct` set on a video lesson).
  - **Frontend JS (`assets/js/frontend.js`, now enqueued globally as `slms-frontend`):** rewritten to (1) follow the completion redirect, (2) video gating — poll for the Presto/HTML5 media element and enable the Complete button at N% watched (fail-open after ~20s), (3) quiz countdown timer — injected on quiz lessons by `lms-content` (`_lms_quiz_timer`), disables the GF submit button + shows a retake notice on expiry. Per-module enqueue removed from `lms-complete-button` to avoid double-loading.
  - **Student Dashboard:** new "My Courses" tab inserted before Purchase History, reusing `Access::get_enrolled_courses_with_progress()`.
  - **Admin React:** `LessonSettings.js` gains drip days, quiz passing-score field/min, and video-gate percent; `CourseEditor.js` gains the completion redirect URL. Bundle rebuilt (`npm run build`).
...
- **Certificate PDF Conditional Logic Fix (Apr 2026):** Diagnosed and resolved three compounding root causes that caused all certificate PDF links to show "N/A":
  1. **`class-certificates.php` — three bugs in `check_course_completion()`:**
     - `GFAPI::get_entries()` was passed only the `field_filters` sub-array instead of a full `$search_criteria` object, so the dedup check always returned empty and created duplicate GF entries on every completion.
     - Auto-generated GF entries were created with only `form_id`, `created_by`, and `status` — field `6` (State) and field `18` (Course URL) were never populated. `GPDFAPI::get_entry_pdfs()` returned empty for every such entry because no PDF template's conditional logic could match.
     - `CourseHistory::insert()` was called without `$entry_id` or `$form_id`, so every new-completion history row had `gf_entry_id = NULL`. The dashboard PDF block is gated on `if ($gf_entry_id && $pdf_form_id)` — NULL skips it entirely, producing permanent N/A.
     - **Fix:** Pass a proper `$search_criteria` object to `get_entries()`; populate fields `'6'` and `'18'` with `get_user_meta($user_id, 'billing_state', true)` and `get_permalink($course_id)` when creating the entry; capture the returned entry ID and pass it with `$form_id` to `CourseHistory::insert()`. Also link existing entries when one is found (sets `$linked_entry_id = $entries[0]['id']`).
  2. **`slms-student-dashboard/includes/frontend.php` — backfill wrote wrong value into field 18:**
     - `slms_course_history.course_name` is stored as a plain title string (e.g., `"8 Hours Laser Hair Removal Continuing Education"`) for new completions. The else branch in course resolution never resolved `$course_link`, so `$url_to_match = '' ?: title_string`. The backfill then wrote the human-readable title into GF field 18, which GravityPDF's URL-based conditions cannot match. Additionally, the backfill was conditioned on `empty(field)` — entries with a wrong (non-empty) field 18 value were never corrected.
     - **Fix:** Added a `get_posts(['title' => $raw_course_name, 'post_type' => 'slms_course'])` lookup in the else branch to resolve `$course_link` from the title. Changed the backfill to always overwrite field 6 (state) and to use `$course_link` (permalink) for field 18 when available, with the raw value as fallback only for empty fields.
  3. **Stage 2 URL matching in both `frontend.php` and `class-rest.php`:**
     - `strpos` is case-sensitive — a title slug like `"laser-hair-removal"` would never match against a stored course title like `"Laser Hair Removal"`.
     - When the PDF condition URL path contained no `"course"` segment, `$course_slug` was forced to `''` and `$match` was always `false` — Stage 2 silently failed for those templates.
     - **Fix:** Changed to `stripos` throughout; added a last-path-segment fallback when `array_search('course', ...)` returns false; added `sanitize_title($course_name)` comparison so a stored plain-text title converts to the URL slug expected by the condition value.
...
- **Certificate PDF Link Fix (Apr 2026):**
  ...
  - **Follow up Active Bug:** PDF rendering still showed N/A because `true === $pdf_config['active']` failed strict boolean checks against the string `'1'` returned from Gravity PDF API. Relaxed check to `!empty($pdf_config['active'])` which restored functionality.
...
- **Certificate PDF Conditional Logic Fix (Apr 2026):**
  - **Root cause:** The URL was being generated correctly (right hash selected), but GravityPDF re-evaluates conditional logic server-side when serving the download. Migrated GF entries are missing field 6 (State) and field 18 (Course URL), so the server-side check always fails with "PDF conditional logic requirements have not been met."
  - **Fix in `slms-student-dashboard/includes/frontend.php`:** When Stage 1 (`GPDFAPI::get_entry_pdfs()`) returns empty, fetch the GF entry and backfill field 6 from `billing_state` user meta and field 18 from the resolved course permalink using `GFAPI::update_entry_field()`. Retry Stage 1 immediately after. Stage 2 (manual conditional logic evaluation) is now a true last resort only reached if the entry fields could not be populated.
  - `$student_state` and `$url_to_match` moved before Stage 1 so they are available for both the backfill and Stage 2.
- **Concatenated Course ID Bug (Apr 2026):**
  - **Root Cause & Fix:** Certain migration records manually entered in Gravity Forms resulted in comma-less concatenated integers like `546630` (from `546` and `630`). Created `extract_legacy_course_ids` in `class-migration.php` Phase 2 loops to proactively split integers of length 5 and 6 into valid mapped components instead of skipping them.
