...
- **Stage 1 (Content Guarding & Student Access Model)** made content actually
  protected: a single `Access` authority (`can_view` / `denial_reason`), three
  enforcement layers (`template_redirect`, `the_content` @99, REST content
  stripping), per-course guard config (`_lms_guard_mode` + denial behavior +
  checkout override), a session-scoped `/me/*` REST namespace, the queryable
  `wp_slms_lesson_progress` table (schema step 2, dual-write + backfill), and a
  React Settings screen. See `CHANGELOG.md` for details.
...
| `wp_slms_lesson_progress` | Queryable lesson completions (`user_id`, `course_id`, `lesson_id`, `completed_at`) | `Progress::create_table()` |
...
- `Upgrade` (`includes/class-upgrade.php`) runs on `admin_init` and on
  ...
  schema without reactivation. Step 2 adds `wp_slms_lesson_progress`.
