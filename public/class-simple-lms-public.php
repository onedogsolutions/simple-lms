...
class SimpleLMS_Public extends SimpleLMS_Common
{
  ...
  public function enqueue_styles()
  {
    if (is_user_logged_in()) {
      if (!function_exists('is_plugin_active') || is_plugin_active('optimizePressPlugin/optimizepress.php')) {
        ...
        wp_enqueue_style($this->plugin_name);
        if ($bar_theme !== 'false')
          wp_enqueue_style($this->plugin_name . '-bar-graph');
        if ($radial_theme !== 'false')
          wp_enqueue_style($this->plugin_name . '-radial-graph');
      }
    }
  }
  ...
  public function enqueue_scripts()
  {
    if (is_user_logged_in()) {
      ...
      wp_localize_script($this->plugin_name, 'simple_lms_vars', $params);
    }
  }
  ...
  public function complete_button_cb($atts, $content = null, $tag = '')
  {
    ...
    if (!is_user_logged_in()) {
       ...
       */
      do_action('simple_lms_show_button_failed', 'guest', $atts, $content, $tag);
     ...
     * @param string       $tag     The shortcode tag name.
     */
    $atts = apply_filters('simple_lms_button_attrs', $atts, $content, $tag);
    ...
    if (!in_array(get_post_type($post_id), $this->get_enabled_post_types())) {
       ...
       * @param string       $tag     The shortcode tag name.
       */
      do_action('simple_lms_show_button_failed', 'unsupported', $atts, $content, $tag);
    ...
    if (!$this->post_can_complete($post_id)) {
       ...
       * @param string       $tag     The shortcode tag name.
       */
      do_action('simple_lms_show_button_failed', 'not_completable', $atts, $content, $tag);
    ...
    if (isset($atts['hidden'])) {
       ...
       * @param string       $tag     The shortcode tag name.
       */
      do_action('simple_lms_show_button_failed', 'hidden', $atts, $content, $tag);
    ...
    if (isset($atts['class']) && !empty($atts['class'])) {
      $custom_classes = sanitize_html_class($atts['class']);
    }
    $custom_styles = false;
  ...
  public function progress_percentage_cb($atts, $content = null, $tag = '')
  {
    ...
    if (!is_user_logged_in()) {
       ...
       * @param string       $tag     The shortcode tag name.
       */
      do_action('simple_lms_show_progress_failed', 'guest', $atts, $content, $tag);
     ...
     * @param string       $tag     The shortcode tag name.
     */
    $atts = apply_filters('simple_lms_progress_percentage_attrs', $atts, $content, $tag);

    // find the current course's
    if ((!isset($atts['course']) || empty($atts['course'])) && is_numeric(get_the_ID())) {
      if ($post_course = $this->post_course(get_the_ID()))
        $atts['course'] = $post_course;
    }
  ...
  public function progress_ratio_cb($atts, $content = null, $tag = '')
  {
    ...
    if (!is_user_logged_in()) {
       ...
       * @param string       $tag     The shortcode tag name.
       */
      do_action('simple_lms_show_progress_ratio_failed', 'guest', $atts, $content, $tag);
     ...
     * @param string       $tag     The shortcode tag name.
     */
    $atts = apply_filters('simple_lms_progress_ratio_attrs', $atts, $content, $tag);
    ...
    if (count($total_buttons) <= 0) {
       ...
       * @param string       $tag     The shortcode tag name.
       */
      do_action('simple_lms_show_progress_ratio_failed', 'no_button', $atts, $content, $tag);
  ...
  public function progress_radial_graph_cb($atts, $content = null, $tag = '')
  {
    if (!is_user_logged_in()) {
       ...
       * @param string       $tag     The shortcode tag name.
       */
      do_action('simple_lms_show_progress_radial_graph_failed', 'guest', $atts, $content, $tag);
    ...
    if ($radial_theme === 'false') {
       ...
       * @param string       $tag     The shortcode tag name.
       */
      do_action('simple_lms_show_progress_radial_graph_failed', 'disabled', $atts, $content, $tag);
      return;
    }
    $loading = false;
  ...
  public function progress_bar_graph_cb($atts, $content = null, $tag = '')
  {
    if (!is_user_logged_in()) {
       ...
       * @param string       $tag     The shortcode tag name.
       */
      do_action('simple_lms_show_progress_graph_failed', 'guest', $atts, $content, $tag);
    ...
    if ($bar_theme === 'false') {
       ...
       * @param string       $tag     The shortcode tag name.
       */
      do_action('simple_lms_show_progress_graph_failed', 'disabled', $atts, $content, $tag);
  ...
  public function append_custom_styles()
  {
    if (!is_user_logged_in()) {
       ...
       * @param string       $reason  The reason, why the function failed.
       */
      do_action('simple_lms_custom_styles_failed', 'guest');

      return;
    }
    if (!SIMPLE_LMS_IS_ACTIVATED) {
      /**
       ...
       * @param string       $reason  The reason, why the function failed.
       */
      do_action('simple_lms_custom_styles_failed', 'disabled');
...
';
    ...
    if ($bar_theme === 'classic')
      $graph_styles .= $bar_styles;
    if ($radial_theme === 'classic')
      $graph_styles .= $radial_styles;
  ...
  public function mark_completed()
  {
    ...
    // Mark this button as completed:
    if (!isset($user_completed[$unique_button_id]))
      $user_completed[$unique_button_id] = array();
    ...
    if (isset($_POST['class']) && !empty($_POST['class'])) {
      $button_class = sanitize_html_class($_POST['class']);
    }
    ...
    do_action('simple_lms_mark_completed', array('user_id' => get_current_user_id(), 'post_id' => $post_id, 'button_id' => $unique_button_id, 'course' => $course));
    do_action('simple_lms_button_completed', array('user_id' => get_current_user_id(), 'post_id' => $post_id, 'button_id' => $unique_button_id, 'course' => $course));
    if ($this->post_completion_status($post_id) == 'completed') {
      do_action('simple_lms_page_completed', array('user_id' => get_current_user_id(), 'post_id' => $post_id, 'button_id' => $unique_button_id, 'course' => $course));
    }
    if ($this->course_completion_status($course) == 'completed') {
      do_action('simple_lms_course_completed', array('user_id' => get_current_user_id(), 'post_id' => $post_id, 'button_id' => $unique_button_id, 'course' => $course));
    }
  ...
  public function mark_uncompleted()
  {
    ...
    // Add action for other plugins to hook in:
    do_action('simple_lms_mark_incomplete', array('user_id' => get_current_user_id(), 'post_id' => $post_id, 'button_id' => $unique_button_id, 'course' => $course));
    do_action('simple_lms_button_uncompleted', array('user_id' => get_current_user_id(), 'post_id' => $post_id, 'button_id' => $unique_button_id, 'course' => $course));
    if ($previous_post_status == 'completed') {
      do_action('simple_lms_page_uncompleted', array('user_id' => get_current_user_id(), 'post_id' => $post_id, 'button_id' => $unique_button_id, 'course' => $course));
    }
    if ($previous_course_status == 'completed') {
      do_action('simple_lms_course_uncompleted', array('user_id' => get_current_user_id(), 'post_id' => $post_id, 'button_id' => $unique_button_id, 'course' => $course));
    }
  ...
  {
    if (!is_user_logged_in())
      return; // dont show conditional content for logged out users
    $atts = array_change_key_case((array)$atts, CASE_LOWER); // normalize attribute keys, lowercase
  ...
  public function build_post_criteria($args = array())
  {
    ...
    if (count($courses) > 1) {
      ...
      foreach ($completable_posts as $post_id => $post) {
        if (!isset($post['course']) || ($post['course'] === 'true'))
          $post['course'] = get_bloginfo('name');
  ...
  public function list_completable_shortcode($atts, $content = null, $tag = '')
  {
    ...
    if (!isset($r['hierarchical']) && ($r !== false))
      $r = current($r);
  ...
  public function next_to_complete_shortcode($atts, $content = null, $tag = '')
  {
    ...
    foreach ($page_ids_in_order as $page_id) {
      ...
      if (!$this->page_is_completed($page_id)) {
        ...
        $prepend = '';
        if (isset($args['prepend']) && !empty($args['prepend']))
          $prepend = $args['prepend'];
        $append = '';
        if (isset($args['append']) && !empty($args['append']))
          $append = $args['append'];
    ...
    // if has attribute "json", return a json string
      ...
      $not_found = '';
      if (isset($args['not_found']) && !empty($args['not_found']))
        $not_found = sanitize_text_field($args['not_found']);
  ...
  public function last_completed_shortcode($atts, $content = null, $tag = '')
  {
    ...
    if (count($completed_posts) > 0) {
      ...
      $prepend = '';
      if (isset($args['prepend']) && !empty($args['prepend']))
        $prepend = sanitize_text_field($args['prepend']);
      $append = '';
      if (isset($args['append']) && !empty($args['append']))
        $append = sanitize_text_field($args['append']);
  ...
  public function peer_pressure_shortcode($atts = array(), $content = null, $tag = '')
  {
    ...
    if (isset($atts['course']) && !empty($atts['course'])) {
      ...
      if (isset($atts['plural']) && !empty($atts['plural']))
        $plural = $atts['plural'];
      $completed = "Yay, you've completed this course!";
      if (isset($atts['completed']))
        $completed = $atts['completed'];
      if ($completed == 'false')
        $completed = '';
  ...
  {
    if (!is_user_logged_in())
      return; // dont show for logged out users
    $atts = array_change_key_case((array)$atts, CASE_LOWER); // normalize attribute keys, lowercase
  ...
  public function reset_account()
  {
    ...
    if (isset($_REQUEST['course']) && !empty($_REQUEST['course'])) {
      ...
      if ($saved) {
        ...
        $updates_to_sendback['.all-courses[data-progress]'] = $this->get_percentage(array('course' => 'all'));
        // Add action for other plugins to hook in:
        do_action('simple_lms_reset_course', array('user_id' => $user_id, 'course' => $course));
      }
    }
    else {
      ...
      if ($this->get_user_activity($user_id) == array()) {
        ...
        if ($saved) {
          ...
          // Add action for other plugins to hook in:
          do_action('simple_lms_reset', array('user_id' => $user_id));
        }
